...
class WebBridge(private val act: Activity) {
    ...
    private fun backupChanged() { (act as? MainActivity)?.backupChanged() }
    @JavascriptInterface fun chooseNotificationVibration(){NotificationPreferences.choose(act)}
    @JavascriptInterface fun notificationPreferences():String=NotificationPreferences.state(act).toString()

    @JavascriptInterface fun syncRequest(id:String,text:String){CloudSyncHttp.request(act as MainActivity,id,text)}
    @JavascriptInterface fun syncCredentials() {
        if (!CloudBackup.configured(act)) { (act as? MainActivity)?.syncCredentials(null); return }
        val user=com.google.firebase.auth.FirebaseAuth.getInstance().currentUser
        if(user==null) { (act as? MainActivity)?.syncCredentials(null); return }
        user.getIdToken(false).addOnSuccessListener { result ->
            val opts=com.google.firebase.FirebaseApp.getInstance().options
            val value=JSONObject().put("uid",user.uid).put("token",result.token)
                .put("project",opts.projectId).put("bucket",opts.storageBucket)
            (act as? MainActivity)?.syncCredentials(value)
        }.addOnFailureListener { (act as? MainActivity)?.syncCredentials(null) }
    }
    @JavascriptInterface fun backupStatus(): String = Backups.status(act).toString()
    @JavascriptInterface fun saveSnapshot(text: String): Boolean = Backups.snapshot(act, text)
    @JavascriptInterface fun chooseBackupFolder() { (act as? MainActivity)?.chooseBackupFolder() }
    @JavascriptInterface fun configureBackup(days: Int) {
        try { Backups.configure(act, days) }
        catch(e: Exception) { Backups.prefs(act).edit().putString("error", e.message).apply() }
        backupChanged()
    }
    @JavascriptInterface fun backupNow() {
        Backups.executor.execute { Backups.local(act.applicationContext); backupChanged() }
    }
    @JavascriptInterface fun cloudSignIn(email: String, password: String, create: Boolean) {
    ...
    @JavascriptInterface fun cloudRestore(id: String) {
        CloudBackup.restore(act, id, ::backupChanged) { (act as? MainActivity)?.restoreCloud(it) }
    }
    ...
    fun status(): String {
        val o = JSONObject()
        o.put("native", true)
        o.put("locationRunning",LocationMonitor.running)
        o.put("locationError",NativeRepo.prefs(act).getString("locationError",""))
    ...
    fun openChannelSettings() = act.runOnUiThread {
        try {
            act.startActivity(Intent(Settings.ACTION_CHANNEL_NOTIFICATION_SETTINGS)
                ...
                .putExtra(Settings.EXTRA_CHANNEL_ID, NotificationPreferences.channelId(act)))
        } catch (e: Exception) {
            try {
